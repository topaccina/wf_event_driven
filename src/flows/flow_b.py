...
def flow_b(random_number: int = None):  # random_number is passed automatically by Prefect automation       # type: ignore  # noqa: E501        
    """
    ...
    if random_number is None:
        print("⚠️  Warning: No random number received in event payload")
        print("Flow B completed (no data to process)")
        return None
    
    print(f"✅ Received event 'italy.random.number.generated'")
    print(f"📊 Random number from Flow A: {random_number}")
    
    # Process the event data
    result = process_event_data(random_number)
    
    print(f"✅ Event data processed successfully")
